...
class MultiLoraLoader:
    ...
    def INPUT_TYPES(s):
        return {"required": { "model": ("MODEL",),
                              ...
                              "text": ("STRING", {
                                "multiline": True,
                                "default": ""}),
                              "lora_stack": ("LORA_STACK",),  # optional input
    ...
    def load_loras(self, model, clip, text, lora_stack=None):
        result = (model, clip)

        # 1️⃣ Apply loras from incoming stack if available
        if lora_stack:
